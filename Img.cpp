            ...
            for ( j = 0; j < nWidth; j++ )
                {
                    ...
                    grayRet.SetPixel ( j, i, RGB ( pixel, pixel,
                                                   pixel ) );
                }
        }
    ...
    for ( int i = 0; i < m_pBMIH->biHeight; i++ )
        {
            ...
            memcpy ( m_lpData[i], gray.m_lpData[i],
                     nWidthBytes );
        }
}
...
BOOL CImg::AttachFromFile ( CFile &file )
{
    ...
    // 读取信息头
    pBMIH = ( BITMAPINFOHEADER* ) new
            BYTE[bmfHeader.bfOffBits - sizeof ( bmfHeader )];
    ...
    int nWidthBytes = WIDTHBYTES ( ( pBMIH->biWidth )
                                   * pBMIH->biBitCount );
    ...
    for ( int i = 0; i < m_pBMIH->biHeight; i++ )
        {
            ::SetDIBitsToDevice ( *pDC, 0, 0,
                                  m_pBMIH->biWidth,
                                  m_pBMIH->biHeight, 0, 0, i, 1, m_lpData[i],
...
BOOL CImg::Draw ( CDC* pDC, CRect rt )
{
    ...
    for ( int i = 0; i < m_pBMIH->biHeight; i++ )
        {
            ::SetDIBitsToDevice ( *pDC, rt.left, rt.top,
                                  rt.Width(),
                                  rt.Height(), 0, 0, i, 1, m_lpData[i],
                                  ( BITMAPINFO* ) m_pBMIH, DIB_RGB_COLORS );
        }
...
/**************************************************
    ...
    无
***************************************************/
void CImg::SetPixel ( int x, int y,
                      COLORREF color )
{
    if ( m_pBMIH->biBitCount == 8 )         // 256色图
        {
            m_lpData[m_pBMIH->biHeight - y - 1][x] =
                GetRValue ( color );
        }
            ...
            if ( m_pBMIH->biBitCount == 24 )
                {
                    ...
                    m_lpData[m_pBMIH->biHeight - y - 1][x * 3 + 1] =
                        GetGValue ( color );
                    m_lpData[m_pBMIH->biHeight - y - 1][x * 3 + 2] =
                        GetRValue ( color );
                }
}
...
void CImg::Line ( POINT ptStart, int nLen,
                  int nWide, BOOL bHor )
{
    int i, j;
    DWORD dw = RGB ( 0, 0, 0 );
...
void CImg::Line ( POINT ptStart, POINT ptEnd )
{
    CVector2D line;
    // 获取距离
    int l = ( int ) ( sqrt ( ( double ) (
                                 ptStart.x - ptEnd.x ) * ( ptStart.x - ptEnd.x )
                             + ( ptStart.y - ptEnd.y ) * ( ptStart.y -
                                     ptEnd.y ) ) );
    ...
    if ( m_pBMIH->biBitCount == 24 )
        {
            for ( int i = 0; i <= l; i++ )
                {
                    line.GetDiscretionalNextPos ( ptStart, ptEnd, l,
                                                  i, ptNext );
                    SetPixel ( ptNext.x, ptNext.y, RGB ( 255, 0,
                                                         0 ) );
                }
        }
