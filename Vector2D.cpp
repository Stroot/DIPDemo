...
void CVector2D::GetDiscretionalNextPos (
    POINT ptStart,
    POINT ptEnd, int nTotal, int nNext,
    POINT& ptNext )
{
    if ( !nTotal ) { return; }
    ...
    POINT ptOnLine, int nLenght, POINT &ptNext )
{
    ...
    // 确定直线方程  斜率k，纵轴交点b
    float k = ( float ) ( ptOnLine.y - ptStart.y ) /
              ( float ) ( ptOnLine.x - ptStart.x );
    ...
    POINT ptOnLine, int nLenght, POINT &ptNext )
{
    ...
    // 确定直线方程  斜率k，纵轴交点b
    float k = ( float ) ( ptOnLine.x - ptStart.x ) /
              ( float ) ( ptOnLine.y - ptStart.y );
