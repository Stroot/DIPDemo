...
class CImg {
    ...
    public:
        ...
        void Line ( POINT ptStart, int nLen, int nWide,
                    BOOL bHor );
        // 绘制矩形
...
inline COLORREF CImg::GetPixel ( int x, int y )
{
    if ( m_pBMIH->biBitCount == 8 )     // 256色图
        {
            BYTE byte = m_lpData[m_pBMIH->biHeight - y -
                                 1][x];
        ...
        if ( m_pBMIH->biBitCount == 1 ) // 单色图
            {
                BYTE ret = ( 1 << ( 7 - x % 8 ) &
                             m_lpData[m_pBMIH->biHeight - y - 1][x / 8] );
                // 令0代表黑
            ...
            if ( m_pBMIH->biBitCount == 24 ) // 真彩图
                {
                    COLORREF color = RGB ( m_lpData[m_pBMIH->biHeight
                                                    - y - 1][x * 3 + 2],
