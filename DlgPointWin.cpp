...
CDlgPointWin::CDlgPointWin ( CWnd*
                             pParent /*=NULL*/ )
...
void CDlgPointWin::OnLButtonDown ( UINT nFlags,
                                   CPoint point )
{
    // 当用户单击鼠标左键开始拖动
...
void CDlgPointWin::OnMouseMove ( UINT nFlags,
                                 CPoint point )
{
    // 判断当前光标是否在绘制区域
    if ( m_MouseRect.PtInRect ( point ) )
...
void CDlgPointWin::OnLButtonUp ( UINT nFlags,
                                 CPoint point )
{
    // 当用户释放鼠标左键停止拖动
    if ( m_iIsDraging != 0 )
